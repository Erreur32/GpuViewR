...
export default function StatsSection({ gpuIndex }: Props) {
  ...
  const rows = [
    ...
    // on this host (e.g. fan_speed on a passive iGPU), the row should
    // fade to muted gray instead of the default green "ok" state that
  ...
  return (
    <div className="space-y-4">
      ...
      <details className="card p-0 overflow-hidden">
        <summary
          className="cursor-pointer select-none px-4 py-3 text-sm font-medium flex items-center justify-between"
        ...
        <table className="w-full text-sm">
          <thead>
            ...
            >
              <th className="text-left py-2 px-4 font-medium uppercase text-xs tracking-wider">
                {t("dashboard.metric")}
              </th>
              <th className="text-right py-2 px-4 font-medium uppercase text-xs tracking-wider">
                {t("dashboard.min")}
              </th>
              <th className="text-right py-2 px-4 font-medium uppercase text-xs tracking-wider">
                {t("dashboard.avg")}
            ...
            {rows.map((row) => (
              <tr
                ...
                style={{
                  ...
                  // Whole row dims when the metric is N/A — covers the
                  // 3 value cells via inheritance, matches the muted
                  // card color above so the table reads as one piece.
                  color: row.available ? undefined : "var(--gv-text-dim)",
...
function Cell({
  ...
  return (
    <div className="text-center">
      ...
      <div
        className={"tabular-nums " + (bold ? "font-bold text-sm" : "text-xs")}
