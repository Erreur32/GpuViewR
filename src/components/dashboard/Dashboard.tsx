...
export default function Dashboard() {
  ...
  const samples = Array.from(latest.values()).sort(
    (a, b) => a.gpu_index - b.gpu_index,
  );
  ...
  const currentHostLabel =
    currentHost?.label || currentHost?.hostname || selectedHostId.slice(0, 13);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        <h2
          className="text-xl font-semibold leading-none inline-flex items-center gap-2"
          style={{ color: "var(--gv-text)" }}
        >
          <VendorIcon
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <GaugeCard
          ...
          ts={active.timestamp_epoch}
          available={active.memory_used != null && !!active.memory_total}
        />
        <GaugeCard
          variant={gaugeView}
      ...
      <GpuProcessesTable
        gpuIndex={active.gpu_index}
        hostId={selectedHostId}
        gpuUtilFallback={active.utilization ?? null}
      />

      <h3
        className="text-sm font-semibold uppercase tracking-wider pt-2"
        style={{ color: "var(--gv-text-muted)" }}
      >
...
function PcieBandwidthCard({ sample }: Readonly<{ sample: GpuSample }>) {
  const { t } = useTranslation();
  const linkBw = pcieBandwidth(
    sample.pcie_gen_current,
  ...
  const link =
    sample.pcie_gen_current && sample.pcie_width_current
  ...
  const degraded =
    sample.pcie_width_current !== null &&
    sample.pcie_width_current !== undefined &&
    sample.pcie_width_max !== null &&
    sample.pcie_width_max !== undefined &&
    ...
    <details open className="card p-0 overflow-hidden">
          ...
          {degraded && (
            <span
              ...
              style={{
                color: "var(--gv-warn)",
                background:
                  "color-mix(in srgb, var(--gv-warn) 15%, transparent)",
                border:
                  "1px solid color-mix(in srgb, var(--gv-warn) 35%, transparent)",
              }}
            >
      ...
      <div className="px-4 pb-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <PcieLinkBwTile
          value={linkBw}
          max={linkMax}
          label={t("dashboard.pcie_link_bw")}
        />
        <div
...
function HostChip({
  label,
  isLocal,
}: Readonly<{ label: string; isLocal: boolean }>) {
  return (
    <span
