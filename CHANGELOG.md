...
- **Per-host color picker on Settings → Hosts.** `/fleet`'s chart and legend colors used to be purely index-based (a host's position in the list). A new swatch button next to each host's label opens a popover with the 10 palette colors, a native custom-color picker, and an "Auto" reset — the chosen color is now pinned to that host everywhere (legend swatch, chart line, tooltip) instead of shifting if the fleet composition changes.
- Default palette's 3rd color changed from orange to a mauve/fuchsia (`#c026d3`).
...
- **README emoji diet.** User feedback: too many emojis read as
  unprofessional. Stripped emojis from demo links (🔴 / 🛰️),
  feature list (🟢 🛰️ 🔁 🔢 🚨 📤 🌍 🐳), and the four
  collapsible section headers (⚙️ 🛰️ 🏗️ 🔧). Kept only the
  🧠 in the AI/LLM banner since it's the brand identity beat.
  No content lost; just the decorative chrome.
