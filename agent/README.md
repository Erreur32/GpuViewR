...
A drop-in `compose.agent.nvidia.yaml` lives at the project root. Copy it to the remote host with a tiny `.env`:
...
docker compose -f compose.agent.nvidia.yaml logs -f
```
