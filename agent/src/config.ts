  ...
  try {
    ...
    if (u.protocol !== "ws:" && u.protocol !== "wss:") {
      process.stderr.write(
        `[FATAL] HUB_URL must be ws:// or wss:// (got: ${u.protocol})\n`,
      );
      process.exit(1);
    }
    if (u.protocol === "ws:") {
      ...
      if (isIpv4 || isIpv6) {
        ...
        if (!isLoopback && !isPrivate) {
          process.stderr.write(
            `[WARN] HUB_URL uses ws:// against a public IP (${host}). Use wss:// in production.\n`,
          );
        }
      }
    }
 ...
 */
export function parseHubTargets(
  ...
  if (hasMulti && hasSingle) {
    process.stderr.write(
      "[FATAL] HUB_URLS and HUB_URL are both set. Pick one — multi-hub mode (HUB_URLS) or single-hub (HUB_URL).\n",
    );
    process.exit(1);
  }

  if (hasMulti) {
    const urls = (env.HUB_URLS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const ids = (env.HOST_IDS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const tokens = (env.AGENT_TOKENS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (urls.length === 0) {
    ...
    if (urls.length !== ids.length || urls.length !== tokens.length) {
      process.stderr.write(
        `[FATAL] HUB_URLS / HOST_IDS / AGENT_TOKENS arrays must have the same length (got ${urls.length}/${ids.length}/${tokens.length}).\n`,
      );
      process.exit(1);
    }
...
export function loadConfig(): AgentConfig {
  ...
  return {
    ...
    tickMs,
    // PCIe RX/TX throughput is the only metric pulled from `nvidia-smi -q`
    // (a heavy ~150 KB full-driver dump, ~50-80 ms CPU per spawn). Driver
    // updates that field a couple of times per second internally; refreshing
    // it at 1 Hz alongside the cheap `--query-gpu=...` cadence doubled the
    // agent's host CPU. Decoupling it to 5 s drops total fork cost ~70%
